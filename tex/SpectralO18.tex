...
\begin{document}
  ...
  \begin{abstract}
    ...
    structure settles.
    We prove that the Born--Infeld action $S[\chifield]$ is even in $\chifield$, and that the
    parity map acting simultaneously on configuration and probe,
    $(\chifield,\eta)\mapsto(-\chifield,-\eta)$, preserves every effective response
    functional: parity is an \emph{equivariant} symmetry of the Born--Infeld structure.
    ...
    Consequently, the parity orbit $\{\chifield,-\chifield\}$ is a candidate fibre supplied by
    an equivariance convention, not a fibre forced by the action alone.
  ...
  \textbf{Keywords.}
  fibre-level admissibility, Born--Infeld parity, equivariant symmetry, parity involution,
  ...
  \approx 7.44$.
    ...
    \paragraph{Scope and what this paper does and does not establish.}
      ...
      (ii) parity acting simultaneously on configuration and probe is an equivariant
      symmetry of every Born--Infeld response;
      ...
      response family, and a factorisation theorem for $\Proj$ --- are stated here
      explicitly and remain unsupplied.
  ...
  \subsection{Two notions of indiscernibility}
    ...
    \begin{proof}
      Take a one-dimensional configuration space, $D = 1$, and the even response
      $R(x) = x^{2}$.
      ...
      \]
      Evenness gives only the reindexed family equality
      ...
      so any response resolving that cross term separates the two configurations under the
      common probe $\eta$.
    \end{proof}

    \begin{definition}[Equivariant BI-indiscernibility]
      \label{def:equivariant}
      Let $g$ be a transformation acting on configurations and perturbations.
      Two configurations $\chifield_{1}, \chifield_{2}$ are \emph{$g$-equivariantly
      BI-indiscernible} if $\chifield_{2} = g\chifield_{1}$ and, for every admissible
      perturbation $\eta$ and every response functional $R$ as in
      Definition~\ref{def:indiscern},
      \begin{equation}
        R[\chifield_{1} + \epsilon\eta]
        = R[g\chifield_{1} + \epsilon\, g\eta]
        \quad\text{for all admissible } \eta \text{ and all sufficiently small } \epsilon.
        \label{eq:equivariant}
      \end{equation}
      The probe is transported by the same transformation as the configuration.
    \end{definition}

    \begin{proposition}[Parity is an equivariant symmetry]
      \label{prop:equivariance}
      For the parity map $g:\chifield\mapsto-\chifield$, $\eta\mapsto-\eta$, every
      configuration $\chifield$ is $g$-equivariantly BI-indiscernible from $-\chifield$:
      \begin{equation}
        R[\chifield + \epsilon\eta] = R[-\chifield - \epsilon\eta]
    ...
    \begin{proof}
      $-\chifield - \epsilon\eta = -(\chifield + \epsilon\eta)$, and every response
      functional built from $S_{\mathrm{BI}}$ inherits the evenness of
      Lemma~\ref{lem:parity}: $R[-\psi] = R[\psi]$ for all configurations $\psi$.
      Admissibility is preserved because $\|D(-\psi)\| = \|D\psi\|$.
    \end{proof}

    \begin{remark}[What equivariance does and does not give]
      \label{rem:equivariance-scope}
      Proposition~\ref{prop:equivariance} exhibits parity as a symmetry of the Born--Infeld
      response structure under the transported-probe comparison.
      Given a \emph{supplied} group action containing parity, the orbits
      $\{\chifield,-\chifield\}$ define a quotient of configuration space, and a projection
      may consistently be \emph{chosen} to factor through that quotient.
      Equivariance does not prove that the physical projection $\Proj$ has these orbits as
  ...
  \subsection{The fibre-identification problem, typed}
    ...
    \begin{problem}[Fibre identification for the Born--Infeld projection]
      ...
      \begin{enumerate}
        ...
        functionals, with the notion of comparison (fixed-probe or equivariant) fixed once
  ...
  \sigma_c(n) \cdot \sigma_{q-c}(n)$ and stated the identification of $\{c, q-c\}$ with
    ...
    hypothesis.
    The present paper adds the equivariance result (Proposition~\ref{prop:equivariance}),
    which makes the parity orbit the distinguished candidate, and the no-go of
...
that hypothesis: equivariant parity, and no more.
    \end{remark}
  ...
  \begin{enumerate}
    ...
    \item Parity acting simultaneously on configuration and probe is an equivariant
    symmetry of every Born--Infeld response (Proposition~\ref{prop:equivariance}).
    ...
    The parity orbit is therefore supplied by an equivariance convention, not forced by
    the action.
